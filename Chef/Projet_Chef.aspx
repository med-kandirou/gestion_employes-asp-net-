<%@ Page Title="" Language="C#" MasterPageFile="~/Master_pageChef.Master" AutoEventWireup="true" CodeBehind="~/Chef/Projet_Chef.aspx.cs" Inherits="Projet_Fin_Formation.WebForm2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:SqlDataSource ID="SDSAllprjt" runat="server" ConnectionString="<%$ ConnectionStrings:PffConnectionString %>" SelectCommand="SELECT DISTINCT projet.designation, projet.Date_debut_p, projet.Date_fin_p, Entrep_cible.Nom_entrep, Entrep_cible.adresse FROM Entrep_cible INNER JOIN projet ON Entrep_cible.id_projet# = projet.id_projet INNER JOIN Equipe ON projet.id_eq# = Equipe.id_eq WHERE (Equipe.cin_chef# = @cin)">
        <SelectParameters>
            <asp:SessionParameter Name="cin" SessionField="cin" />
        </SelectParameters>
</asp:SqlDataSource>
<asp:SqlDataSource ID="SDSFilterPrjt" runat="server" ConnectionString="<%$ ConnectionStrings:PffConnectionString %>" SelectCommand="SELECT DISTINCT projet.id_projet, projet.id_eq# AS 'Equipe', projet.designation, projet.Date_debut_p AS 'Date Debut', projet.Date_fin_p AS 'Date Fin', Equipe.cin_chef# FROM projet INNER JOIN Equipe ON projet.id_eq# = Equipe.id_eq WHERE (projet.designation = @design) AND (Equipe.cin_chef# = @cin_chef)">
    <SelectParameters>
        <asp:ControlParameter ControlID="DropDownList1" Name="design" PropertyName="SelectedValue" />
        <asp:SessionParameter Name="cin_chef" SessionField="cin" />
    </SelectParameters>
</asp:SqlDataSource>
    <br />
    <br />
